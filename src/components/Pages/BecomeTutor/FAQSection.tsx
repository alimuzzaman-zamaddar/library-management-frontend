...
const FAQSection: React.FC<FAQSectionProps> = ({ data }) => {
  ...
  return (
    <section className="w-full h-auto pb-[120px] gap-y-[59px] flex flex-col  container items-center  ">
      <Heading
        Variant="h4"
      ...
      <div className="w-[80%] flex flex-col gap-y-5  ">
        {data.map((item, index) => (
          <div
            key={index}
            className="bg-white flex shadow-md flex-col border-[1px] border-solid border-alt-border gap-y-4  rounded-[30px] h-auto w-full p-[40px] cursor-pointer ease-in-out duration-300 hover:shadow"
